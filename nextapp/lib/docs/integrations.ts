...
export const INTEGRATIONS: Integration[] = [
  ...
  },
  {
    ...
    confidence: "high",
    completionWarning: "SurveyMonkey appends captured variables to the redirect as a query string (…?messageid=<value>); it cannot build Samply's path-style /studies/<study-code>/done/<id>. Samply accepts that query form too, so give the collector the static base URL /studies/<study-code>/done and let SurveyMonkey append the rest — no rewriter needed. The custom variable must be named messageid.",
    ...
    completionSteps: [
      ...
      "Run one real test interview from a Samply link and confirm the response shows as completed in your Samply data export.",
    ],
    postFeasibility: "external",
    ...
    postSteps: [
      "SurveyMonkey's own webhooks carry only its event ids, never the captured messageid, so a silent POST needs a relay — the browser redirect above needs no such thing and is the simpler choice.",
