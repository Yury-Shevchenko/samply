              ...
              return (
                ...
                >
                  ...
                  <td style={{ ...TD, color: "var(--ink-40)", maxWidth: "20rem" }}>
                    {row.recipientGroupIds.length ? (
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem" }}>
                        {row.recipientGroupIds.map((gid) => (
                          <span key={gid} style={{ fontSize: "0.95rem", padding: "0.15rem 0.6rem", borderRadius: "9999px", background: "var(--ink-10)", color: "var(--ink-60)" }}>
                            {groupNameById[gid] ?? gid}
                          </span>
                        ))}
                      </div>
                    ) : row.recipientUserIds.length ? (
                      <div className="truncate">
                        {row.recipientUserIds.map((id) => participantCodeById[id] ?? id).join(", ")}
                      </div>
                    ) : (
                      <span style={{ color: "var(--ink-40)" }}>all</span>
                    )}
                  </td>
