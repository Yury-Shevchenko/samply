...
import { createCategoryAction } from "@/app/forum/actions";

export const metadata = { title: "Admin: New Forum Category — Samply" };
  ...
  return (
    <main style={{ background: "var(--paper)", minHeight: "100vh", color: "var(--ink)" }}>
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "5.2rem 4rem 9.6rem" }}>
        ...
        <form action={createCategoryAction} style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
          <div>
            <label style={LABEL}>Name</label>
            <input name="name" required maxLength={80} placeholder="e.g. Getting Started" style={INPUT} />
          </div>
          <div>
            <label style={LABEL}>Description <span style={{ textTransform: "none", letterSpacing: 0, color: "var(--ink-40)" }}>(optional)</span></label>
            <input name="description" maxLength={200} placeholder="Short description shown on the forum homepage" style={INPUT} />
          </div>
          <div>
            <label style={LABEL}>Order <span style={{ textTransform: "none", letterSpacing: 0, color: "var(--ink-40)" }}>(lower = first)</span></label>
            <input name="order" type="number" defaultValue="0" style={{ ...INPUT, maxWidth: 120 }} />
          </div>
          <div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", marginTop: "0.8rem" }}>
            ...
            <button type="submit" style={{ padding: "0.9rem 2.2rem", background: "var(--ink)", color: "var(--paper)", borderRadius: "9999px", fontSize: "1.2rem", fontWeight: 500, fontFamily: "var(--font-body)", border: "none", cursor: "pointer" }} className="hover:opacity-80 transition-opacity">Create</button>
          </div>
        </form>
      </div>
    </main>
