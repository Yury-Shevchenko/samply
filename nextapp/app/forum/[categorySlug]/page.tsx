          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem" }}>
            {threads.map((thread) => (
              <a
                ...
                href={`/forum/${categorySlug}/${thread._id}`}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.8rem", flexWrap: "wrap", marginBottom: "0.3rem" }}>
                    {thread.pinned && (
                      <span style={{ fontSize: "1rem", fontFamily: "var(--font-mono)", color: "var(--coral)", background: "rgba(214,90,48,.08)", padding: "1px 7px", borderRadius: 999, letterSpacing: ".06em" }}>{t("forum.pinned")}</span>
                    )}
                    {thread.locked && (
                      <span style={{ fontSize: "1rem", fontFamily: "var(--font-mono)", color: "var(--ink-40)", background: "var(--ink-10)", padding: "1px 7px", borderRadius: 999, letterSpacing: ".06em" }}>{t("forum.locked")}</span>
                    )}
                    {thread.solvedPostId && (
                      <span style={{ fontSize: "1rem", fontFamily: "var(--font-mono)", color: "var(--sage)", background: "rgba(61,115,107,.08)", padding: "1px 7px", borderRadius: 999, letterSpacing: ".06em" }}>{t("forum.solved")}</span>
                    )}
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: "1.1rem", color: "var(--ink-40)", flexShrink: 0, textAlign: "right" }}>
                  <div>{t("forum.replies", { n: thread.replyCount })}</div>
                  <div style={{ fontSize: "1rem", marginTop: "0.2rem" }}>{fmt(thread.lastReplyAt)}</div>
                </div>
              </a>
            ))}
          </div>
