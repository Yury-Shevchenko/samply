...
export async function createGroupAction(formData: FormData) {
  ...
  if (!projectId || !groupName || !participants.length) redirect(`/groups?project=${projectId}`);

  const { p } = await requireOwnerOrMember(projectId);
  ...
  redirect(`/groups?project=${projectId}`);
}
...
export async function deleteGroupAction(formData: FormData) {
  ...
  if (!projectId || !groupId) redirect(`/groups?project=${projectId}`);
