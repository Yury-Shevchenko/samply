...
// ── Express paths use the same expander as the Next.js create routes ────────
...
const joinStudy = body("joinStudy");
...
// The researcher API (/api/notifications) create functions.
for (const name of ["createIntervalNotification", "createIndividualNotification"]) {
  eq(`${name} expands with expandScheduleBetween`, /expandScheduleBetween\(/.test(body(name)), true);
  eq(`${name} no longer expands a raw cron`, /expandCronBetween\(/.test(body(name)), false);
}
