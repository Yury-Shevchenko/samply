...
exports.labjs = async (req, res) => {
  if(req.files.script){
      ...
      const production = req.headers.referer == 'https://labjs-beta.netlify.com/' ? 'beta': 'alpha';
      const json_string = req.files.script[0].buffer.toString();
      const json = JSON.parse(json_string);
      const script = await assemble.convertJSON(json, req.body.name, production = production);
