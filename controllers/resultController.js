    ...
    .on('data', obj => {
      if(obj && obj.data){
        ...
        const line =[ {
          ...
          deleted_by_user: obj.events.filter(e => e.status === 'archived').map(e => e.created.getTime()),
          received_in_app: obj.events.filter(e => e.status === 'received-in-app').map(e => e.created.getTime()),
